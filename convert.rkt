...
(define (anf? exp)
  (cond
   ...
   ((equal? (car exp) 'lambda)
    (let ((body (cddr exp)))
      (and (= (length body) 1) (anf? (car body)))))
   ((equal? (car exp) 'set!)
    (and (symbol? (cadr exp)) (atomic? (caddr exp)) (anf? (caddr exp))))
   ((equal? (car exp) 'if)
    (and (= (length (cdr exp)) 3)
         (atomic? (cadr exp)) (anf? (cadr exp))
         (anf? (caddr exp)) (anf? (cadddr exp))))
   ((equal? (car exp) 'begin)
    #f)
   ((equal? (car exp) 'let*)
    #f)
