...
(define apply-subst-lst #f) ; mutual-recursion
...
(define one-way-unify1 #f)
(define one-way-unify1-lst #f)
(define (one-way-unify term1 term2)
  (begin (set! unify-subst '())
         (one-way-unify1 term1 term2)))

(define rewrite-args #f)
(define rewrite-with-lemmas #f)
(define (rewrite term)
  (cond ((not (pair? term))
         term)
...
;; set up mutually recursive functions
(set! apply-subst-lst (lambda (alist lst)
                        (cond ((null? lst)
                               '())
                              (else (cons (apply-subst alist (car lst))
                                          (apply-subst-lst alist (cdr lst)))))))
(set! trans-of-implies1 (lambda (n)
                          (cond ((equal? n 1)
                                 (list (quote implies)
                                       0 1))
                                (else (list (quote and)
...
(set! rewrite-args (lambda (lst)
                     (cond ((null? lst)
                            '())
                           (else (cons (rewrite (car lst))
                                       (rewrite-args (cdr lst)))))))

(set! rewrite-with-lemmas (lambda (term lst)
                            (cond ((null? lst)
                                   term)
