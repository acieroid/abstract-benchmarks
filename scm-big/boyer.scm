;; Expected result: #t
(letrec ((assq (lambda (k l)
                 (cond ((null? l) #f)
                       ((eq? (caar l) k) (car l))
                       (else (assq k (cdr l)))))))
  (letrec ((member (lambda (v l)
                     (cond ((null? l) #f)
                           ((equal? v (car l)) l)
                           (else (member v (cdr l)))))))
    (let ((*namelist* '()))
                ...
                (let ((reinit-prop! (lambda ()
                  ...
                  (let ((run-benchmark (lambda (benchmark-name benchmark-thunk)
                                         (let ((ten (lambda ()
                                           ...
                                           (ten)
                                           (ten)))))
                    (let ((get-null (lambda (name prop)
                                      (let ((res (get name prop))) (if res res '())))))
                      (let ((unify-subst 0))
                        (let ((temp-temp 0))
                          (let ((add-lemma (lambda (term)
                                             (let ((res (if (pair? term)
                                               ...
                                               (cond (res
                                                      (put (car (cadr term))
                                                           'lemmas
                                                           (cons term (get-null (car (cadr term)) 'lemmas))))
                                                     (else (error 'add-lemma "ADD-LEMMA did not like term:  " term)))))))
                            (letrec ((add-lemma-lst (lambda (lst)
                                                      (cond ((null? lst)
                                                             #t)
                                                            (else (add-lemma (car lst))
                                                                  (add-lemma-lst (cdr lst)))))))
                              (letrec ((apply-subst (lambda (alist term)
                                                      (letrec ((apply-subst-lst (lambda (alist lst)
                                                                                  (cond ((null? lst)
                                                                                         '())
                                ...
                                (let ((falsep (lambda (x lst)
                                  ...
                                  (let ((one-way-unify1 #f))
                                    (let ((one-way-unify1-lst #f))
                                      (let ((one-way-unify (lambda (term1 term2)
                                                             (begin (set! unify-subst '())
                                                                    (one-way-unify1 term1 term2)))))

                                        (let ((rewrite-with-lemmas #f))
                                          (letrec ((rewrite (lambda (term)
                                                              (letrec ((rewrite-args (lambda (lst)
                                                                                       (cond ((null? lst)
                                                                                              '())
                                                                                             (else (cons (rewrite (car lst))
                                                                                                         (rewrite-args (cdr lst))))))))
                                                                (letrec ((rewrite-with-lemmas (lambda (term lst)
                                                                                                (cond ((null? lst)
                                                                                                       term)
                                              ...
                                              (letrec ((tautologyp (lambda (x true-lst false-lst)
                                                ...
                                                (let ((tautp (lambda (x)
                                                  ...
                                                  (let ((test (lambda ()
                                                    ...
                                                    (letrec ((trans-of-implies (lambda (n)
                                                                                 (letrec ((trans-of-implies1 (lambda (n)
                                                                                                               (cond ((equal? n 1)
                                                                                                                      (cons 'implies
                                                                                                                            (cons 0 (cons 1 '()))))
                                                                                                                     (else (cons 'and
                                                                                   ...
                                                                                   (cons 'implies
                                                                                         (cons (trans-of-implies1 n)
                                                                                               (cons (cons 'implies
                                                                                                           (cons 0 (cons n '())))
                                                                                                     '())))))))

                                                      (set! one-way-unify1 (lambda (term1 term2)
                                                                             (cond ((not (pair? term2))
                                                      ...
                                                      (set! one-way-unify1-lst (lambda (lst1 lst2)
                                                                                 (cond ((null? lst1)
                                                                                        #t)
